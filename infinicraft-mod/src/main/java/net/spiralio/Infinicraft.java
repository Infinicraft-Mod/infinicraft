...
public class Infinicraft implements ModInitializer {
    ...
    public static final BlockEntityType<InfinicrafterBlockEntity> INFINICRAFTER_ENTITY = Registry.register(Registries.BLOCK_ENTITY_TYPE, new Identifier("infinicrafter"), FabricBlockEntityTypeBuilder.create(InfinicrafterBlockEntity::new, INFINICRAFTER_BLOCK).build());

    // infinicrafter screen handler
    public static final ScreenHandlerType<InfinicrafterScreenHandler> INFINICRAFTER_SCREEN_HANDLER = Registry.register(Registries.SCREEN_HANDLER, new Identifier("infinicrafter"), new ExtendedScreenHandlerType<>(InfinicrafterScreenHandler::new));

    private ExecutorService iconTalkerExecutor;

    private final LinkedBlockingQueue<IconRequest> requests = new LinkedBlockingQueue<>();

    public record IconRequest(String itemColor, String itemDescription, BiConsumer<IconResponse, Throwable> callback) {
    }

    public record IconResponse(@SerializedName("success") boolean isSuccess,
                               @SerializedName("image") int @Nullable [] image) {
    }

    public static CompletableFuture<IconResponse> IconRequest(GeneratedItem itemData) {
        if (INSTANCE == null) {
        ...
        try {
            INSTANCE.requests.put(new IconRequest(itemData.getColor(), itemData.getName() + " - " + itemData.getDescription(), (response, ex) -> {
                if (ex != null) {
    ...
    public void onInitialize() {
        ...
        iconTalkerExecutor = Executors.newCachedThreadPool(new ThreadFactory() {
            private static final AtomicInteger threadNumber = new AtomicInteger(1);

            @Override
            public Thread newThread(@NotNull Runnable r) {
                Thread t = new Thread(r, "Infinicraft ICON Thread Pool #" + threadNumber.getAndIncrement());
                t.setDaemon(false);
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            ...
            iconTalkerExecutor.execute(() -> {
                ...
                var httpClient = HttpClient.newBuilder().executor(iconTalkerExecutor).build();

                try {
                    while (!Thread.currentThread().isInterrupted()) {
                        var iconRequest = requests.take();
                        var httpRequest = HttpRequest.newBuilder().GET().uri(new URIBuilder(new URI(CONFIG.INFINICRAFT_SERVER() + "/img")).setParameter("itemColor", iconRequest.itemColor()).setParameter("itemDescription", iconRequest.itemDescription()).build()).build();

                        httpClient.sendAsync(httpRequest, GsonBodyHandler.ofJson(JsonHandler.GSON, IconResponse.class)).thenAccept(response -> {
                            if (response.statusCode() >= 400 || response.statusCode() < 200) {
                                var body = response.body().get();
                                LOGGER.error("Icon failure: Status code " + response.statusCode() + "; body: " + body);
