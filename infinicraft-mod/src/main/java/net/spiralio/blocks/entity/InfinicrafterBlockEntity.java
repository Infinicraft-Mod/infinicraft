...
public class InfinicrafterBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory, SidedInventory {
    ...
    private void spawnDustParticles(World world, BlockPos pos) {
        ...
        world.playSound(null, pos, net.minecraft.sound.SoundEvents.BLOCK_FIRE_EXTINGUISH, net.minecraft.sound.SoundCategory.BLOCKS, 1.0F, 1.0F);
    }
    ...
    public static GeneratedItem cleanResponse(String res) {
        ...
        String fixed = extractedJSON.replaceAll("\\bTrue\\b", "true").replaceAll("\\bFalse\\b", "false").replaceAll("\\bNone\\b", "null");

        fixed = fixed.replaceAll("(?<=\\w)'(?=\\w)", "\\\\'");

        return JsonHandler.GSON.fromJson(fixed, GeneratedItem.class);
    }
